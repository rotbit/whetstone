...
graph TB
    ...
    subgraph RUNTIME[Dokploy Compose · 每服务独立容器]
...
**M1 部署方式**：Dokploy 使用一个 Docker Compose 项目管理五个独立容器，每个容器只运行一个 Go 进程。仅 `app-apis:8888` 接入对外域名，三个 RPC 通过 Compose DNS 通信，Worker 独立消费异步任务。这样可以按服务查看日志、限制资源和独立重启，同时用一个 Compose 保持部署操作简单。详细取舍见 `docs/adr/0003-dokploy-multi-container-compose.md`。
