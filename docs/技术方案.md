...
> 版本 v0.1 · 2026-07 · 状态：设计草案
> 技术栈：Go 1.22+ / go-zero / WebSocket / MySQL / Redis / Qdrant / asynq
> 定位：**单人可完成**、面向个人用户（C 端）变现的开源项目；求职展示与商业化兼顾
...
graph TB
    ...
    subgraph ACC[接入层]
        API[api-gateway · REST<br/>用户 · 简历 · 订单 · 报告]
        WS[ws-gateway · WebSocket<br/>心跳 · 重连 · 会话恢复]
    end

    subgraph CORE[核心服务层 · zRPC]
        ENG[interview-rpc<br/>面试引擎 · 状态机 · 追问决策]
        QUE[question-rpc<br/>题库 · RAG 出题]
        USR[user-rpc<br/>用户 · 次数 · 订单]
    end

    ADP[llm-adapter · AI 能力层<br/>LLM · ASR · TTS 多厂商统一封装]
    RPT[report-worker<br/>异步评分 · 报告生成]
...
| 服务 | 类型 | 职责 |
|---|---|---|
| api-gateway | go-zero REST | 用户 / 简历上传 / JD / 订单 / 报告查询 |
| ws-gateway | WebSocket 长连接 | 连接生命周期：心跳、断线重连、会话恢复；音频与文本流转发 |
...
ai-interview/
├── app/
│   ├── api/                     # api-gateway：用户/简历/订单/报告
│   ├── ws/                      # ws-gateway：长连接 + 语音/文本流转发
│   │   └── internal/
│   │       ├── conn/            # 连接管理：心跳/重连/会话恢复
│   │       └── pipeline/        # ASR→LLM→TTS 流式编排、barge-in
│   ├── interview/rpc/           # 面试引擎：状态机/追问决策/上下文
│   ├── question/rpc/            # 题库 + RAG 出题
│   ├── user/rpc/                # 用户/次数/订单/支付回调
│   └── report/worker/           # asynq 消费者：评分与报告
├── common/
