...
部署到 Dokploy 时创建五个独立 Application，共用同一个仓库并分别选择 Dockerfile 的同名构建阶段。生产配置默认通过 Environment 级变量集中管理，也支持 File Mount 覆盖完整 YAML。具体设置见 [deploy/dokploy/README.md](deploy/dokploy/README.md)，MySQL、Redis、Qdrant、MinIO 和阿里云 OSS 的生产配置见 [deploy/dokploy/INFRASTRUCTURE.md](deploy/dokploy/INFRASTRUCTURE.md)。
